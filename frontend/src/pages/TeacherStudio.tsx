...
export default function StudioLayout() {
  ...
  const onNew = () => {
    // نمونه‌ی ساخت دوره جدید: می‌تونی به صفحهٔ Editor روت بدی
    nav("/studio/course/0/edit");
  };
  ...
  return (
    <div className="min-h-screen bg-gray-50 overflow-x-hidden">
      ...
      {/* Sidebar (desktop) */}
      <aside className="hidden md:block fixed top-16 left-0 bottom-0 w-[260px] border-r bg-white overflow-y-auto z-[52]">
        <nav className="p-3">
      ...
      {open && (
        <>
          <div className="fixed inset-0 z-[55] bg-black/30" onClick={() => setOpen(false)} />
          <aside className="fixed z-[56] top-16 left-0 bottom-0 w-[18rem] max-w-[80vw] border-r bg-white overflow-y-auto animate-in slide-in-from-left">
            <nav className="p-3">
              <div className="text-xs uppercase text-gray-400 mb-2">Studio</div>
              <LinkItem to="/studio">Courses</LinkItem>
              <LinkItem to="/studio/lessons">Lessons</LinkItem>
      ...
      {/* Main content wrapper (with left padding on desktop) */}
